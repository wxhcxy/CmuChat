...
void CSession::Close() {
    std::lock_guard<std::mutex> lock(_session_mtx); //保证在任何一个线程关闭，只关闭一次
    _socket.close();
    _b_close = true;
}
...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
            ...
            if (!_server->CheckValid(_session_id)) { //判断session是否有效，无效就关闭掉
                Close();
                return;
            }
...
{
    double diff_sec = std::difftime(now, _last_heartbeat); //两个时间的差值
    if (diff_sec
        > HEART_THRESHOLD) { //如果差值大于我们设置的这个HEART_THRESHOLD 20秒,就表示超时过期了
...
void CSession::DealExceptionSession()
{
    ...
    Defer defer([identifier, lock_key, self, this]() {
        _server->ClearSession(_session_id);
        RedisMgr::GetInstance()->releaseLock(lock_key, identifier); //释放分布式锁
    });

    if (identifier.empty()) { //判断加锁是否成功
        return;
    }
