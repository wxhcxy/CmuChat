...
void LogicSystem::SearchInfo(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	Defer defer([this, &rtvalue, session]() {
		std::string return_str = rtvalue.toStyledString();
		session->Send(return_str, ID_SEARCH_USER_RSP);	//将return_str放入session的发送队列，去发送
		});

	bool b_digit = isPureDigit(uid_str);//isPureDigit函数判断uid_str是数字还是昵称,全是数字返回true
	if (b_digit) {	//是数字，通过数字去获取用户的信息
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	//直接通知对方有申请消息（若对方和自己在同一个服务器里，因为是分布式服务器）
	if (to_ip_value == self_name) {//对方和自己在同一个服务器里
...
void LogicSystem::DealChatTextMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	auto self_name = cfg["SelfServer"]["Name"];
	//直接通知对方有认证通过消息
	if (to_ip_value == self_name) {	//对方和自己在同一个服务器，(判断对方是不是和自己在同一个服务器)
		auto session = UserMgr::GetInstance()->GetSession(touid);
		if (session) {
			//在内存中则直接发送通知对方
			std::string return_str = rtvalue.toStyledString();
