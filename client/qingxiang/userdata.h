...
struct FriendInfo {
    ...
    QString _back;
    QString _last_msg;
    std::vector<std::shared_ptr<TextChatData>> _chat_msgs;  //与好友的聊天记录
};


class TextChatData; //文本消息。以后可能会做其他的消息
...
//可能一下子给服务器发送多条，所以把它做成一个vector，然后放到整体封装好的一个消息里
struct TextChatMsg{
    ...
    int _to_uid;    //从谁到谁
    int _from_uid;
    std::vector<std::shared_ptr<TextChatData>> _chat_msgs;  //包含多条消息
};
#endif
