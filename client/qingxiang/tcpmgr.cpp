...
void TcpMgr::initHandlers()
{
    ...
    //注册登录的传回
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data){ //注册回调lambda
        Q_UNUSED(len);
        ...
        emit sig_swich_chatdlg();   //发送登陆成功，跳转到聊天界面的信号
    });
    ...
    //当我们自己发送完一条消息之后，也是会发给服务器的，服务器会给我们回复一下，我们对这个回复打印一下就可以了
...
{
    uint16_t id = reqId;    //id2字节

    // 计算长度（使用网络字节序转换）
    quint16 len = static_cast<quint16>(dataBytes.size());    //长度2字节
    ...
    qDebug()<<"TcpMgr::slot_send_data dataBytes: "<<dataBytes;
