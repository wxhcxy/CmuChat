    ...
    ,_last_widget(nullptr)
{
    ...
    ui->search_edit->addAction(searchAction,QLineEdit::LeadingPosition);//在搜索栏edit的头部加上搜索图标
    ...
    AddLBGroup(ui->side_chat_lb);//将导航项加到_lb_list里管理
    ...
    //连接好友信息界面发送的聊天点击事件,跳转到和该好友的聊天界面    //就是联系人点击了某个好友然后再点击该好友的聊天按钮
...
void ChatDialog::ShowSearch(bool bsearch)
{
    ...
    if(bsearch){
        ...
        ui->search_list->show();//显示搜索框
        _mode = ChatUIMode::SearchMode;
    }else if(_state == ChatUIMode::ChatMode){ //聊天模式
        ui->chat_user_list->show();//显示聊天记录列表
        ui->con_user_list->hide();
        ui->search_list->hide();
        _mode = ChatUIMode::ChatMode;
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply)
{
    ...
    UserMgr::GetInstance()->AddApplyList(std::make_shared<ApplyInfo>(apply));//如果判断它没有在列表里，就把这个请求加到用户的数据管理里
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    ...
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
    _chat_items_added.insert(auth_info->_uid, item);//通过uid找到对应的item，(QListWidgetItem)
}
